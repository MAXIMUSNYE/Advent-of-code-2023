...
for i in range(0,len(lines)):
    ...
    for char in lines[i]:
            try:
                ...
                continue
    # ad = int(list[0]+list[-1])
    # #print(ad)
    # epic+= ad
#print(epic)
#     all_linesdigits.append(list)
#     epic += int( list[0]+ list[-1])
...
# """check i f char is num if not
# check if first in string if so chec second if not check next in string"""
...
#         try:
#             num = int(car)
#             dict[line].append(car)
#         except ValueError:
#             pass
#     firstdig=key[0]
#     seconddig = key[-1]
#     dict2[line] = int(firstdig + seconddig)
#     products.append(int(firstdig + seconddig))
# sum = 0
...
def get_sumv2(key):
    ...
    val = str(firstdig)+ str(seconddig)
...
finaltoatal = 0 
for i in all_linesdigits:
    finaltoatal += get_sumv2(i)
...
#     total = 0
#     for line in lines:
#         line = replace_spelled_numbers(line.lower())
#         first_digit, last_digit = find_first_and_last_digit(line)
#         print(find_first_and_last_digit(line))
